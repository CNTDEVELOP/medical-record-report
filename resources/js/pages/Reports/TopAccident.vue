...
/* export filename: ACCIDENT_PTTYPE_<PCODE>_<MODE>_<YYYYMMDD_HHmmss> */
const exportFile = computed(() => {
  ...
  return `ACCIDENT_PTTYPE_${code}_${m}_${stamp}`
})
...
const fetchData = async () => {
  ...
  try {
    const params = {
      ...
      pcode : pcode.value || 'AL',
    }
    const { data } = await axios.get('/api/reports/top-accident', { params })
...
<template>
  <div class="max-w-6xl mx-auto px-4 py-6">
    <h1 class="text-xl sm:text-2xl font-semibold text-zinc-800 mb-4">
      อันดับอุบัติเหตุ (ICD-10 S00–S99) — OPD/IPD/รวม
    </h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <div class="flex flex-col">
          <label class="text-xs text-zinc-500 mb-1">สิทธิ (pcode)</label>
          <input type="text" v-model="pcode" placeholder="AL" class="h-[42px] rounded-md border border-zinc-300 px-2 text-sm" />
          <small class="text-[11px] text-zinc-500 mt-1">ค่าเริ่มต้น AL</small>
        </div>
      </div>
