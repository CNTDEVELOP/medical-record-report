...
/* export filename: ACCIDENT_SSS_<MODE>_<YYYYMMDD_HHmmss>.csv */
const exportFile = computed(() => {
  const pad = (n) => String(n).padStart(2, '0')
  const d = new Date()
  const stamp = `${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}`
  const m = (mode.value || 'all').toUpperCase()
  return `ACCIDENT_SSS_${m}_${stamp}`
})
...
const fetchData = async () => {
  ...
  try {
    const params = {
      ...
      // pcode ตรึงเป็น A7,A8 ที่ฝั่ง backend อยู่แล้ว
    }
    const { data } = await axios.get('/api/reports/top-accident-sss', { params })
...
<template>
  <div class="max-w-6xl mx-auto px-4 py-6">
    <h1 class="text-xl sm:text-2xl font-semibold text-zinc-800 mb-4">
      อุบัติเหตุ (ICD-10 S00–S99) — 5 อันดับแรก ของสิทธิ์ประกันสังคม (A7–A8)
    </h1>
