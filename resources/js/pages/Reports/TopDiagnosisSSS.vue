...
const mode  = ref(props.defaultMode || 'all')  // all|opd|ipd
const ipdBy = ref('dchdate')                   // dchdate|regdate
const pcodes = ref(props.defaultPcodes || 'A7,A8')  // ✅ default A7,A8
const limit  = ref(5)                               // ✅ default 5
...
/* export filename: SSS_PTTYPE_<A7A8>_<MODE>_<YYYYMMDD_HHmmss> */
const exportFile = computed(() => {
  const pad = (n) => String(n).padStart(2, '0')
  const d = new Date()
  const stamp = `${d.getFullYear()}${pad(d.getMonth()+1)}${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}`
  const code = (pcodes.value || 'A7,A8').toUpperCase().replace(/[^A-Z0-9]+/g,'') // A7,A8 -> A7A8
  const m    = (mode.value || 'all').toUpperCase()
  return `SSS_PTTYPE_${code}_${m}_${stamp}`
})
...
const fetchData = async () => {
  ...
  try {
    const params = {
      ...
      mode  : mode.value,
      ipd_by: ipdBy.value || 'dchdate',
      limit : clampLimit(limit.value),
      pcodes: pcodes.value || 'A7,A8',
    }
    const { data } = await axios.get('/api/reports/top-diagnosis-sss', { params })
...
<template>
  <div class="max-w-6xl mx-auto px-4 py-6">
    <h1 class="text-xl sm:text-2xl font-semibold text-zinc-800 mb-4">
      โรคที่เข้ารักษา 5 อันดับแรก — สิทธิประกันสังคม (A7–A8)
    </h1>

    <!-- Controls -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
      <div class="flex flex-col">
        <label class="text-xs text-zinc-500 mb-1">วันที่เริ่ม</label>
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <div class="flex flex-col" v-if="mode !== 'opd'">
          <label class="text-xs text-zinc-500 mb-1">IPD นับตาม</label>
          <select v-model="ipdBy" class="h-[42px] rounded-md border border-zinc-300 px-2 text-sm">
            <option value="dchdate">วันจำหน่าย (dchdate)</option>
            <option value="regdate">วัน Admit (regdate)</option>
          </select>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <div class="flex flex-col">
          <label class="text-xs text-zinc-500 mb-1">สิทธิ (pcode) หลายค่า (คั่นด้วย ,)</label>
          <input type="text" v-model="pcodes" placeholder="A7,A8"
                 class="h-[42px] rounded-md border border-zinc-300 px-2 text-sm" />
          <small class="text-[11px] text-zinc-500 mt-1">ค่าเริ่มต้น A7,A8</small>
        </div>
      </div>
    ...
    <div class="rounded-xl border bg-white p-3">
      ...
      >
        ...
        <Column field="icd10" header="ICD-10" exportHeader="ICD-10" />
        <Column field="icd10_name_th" header="ชื่อโรค (ไทย)" exportHeader="ชื่อโรค (ไทย)" />

        <Column v-if="mode !== 'ipd'" field="opd_cases" header="OPD" style="width:120px" exportHeader="OPD"
                :body="row => row.opd_cases?.toLocaleString?.() ?? row.opd_cases" />
        <Column v-if="mode !== 'opd'" field="ipd_cases" header="IPD" style="width:120px" exportHeader="IPD"
                :body="row => row.ipd_cases?.toLocaleString?.() ?? row.ipd_cases" />
        <Column v-if="mode === 'all'" field="total_cases" header="รวม" style="width:120px" exportHeader="รวม"
